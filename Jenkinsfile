        ...
        stage('Build') {
            steps {
                withCredentials([string(credentialsId: 'AWS_SECRET_ID', variable: 'AWS_SECRET_ID'), string(credentialsId: 'AWS_SECRET_KEY', variable: 'AWS_SECRET_KEY')]) {
                    sh 'python3 cloudformation-using-jenkins/cfn_task.py $AWS_SECRET_ID $AWS_SECRET_KEY $STACK_NAME $VPC_CIDR $PUBLIC_SUBNET_CIDR $PRIVATE_SUBNET_CIDR $EMAIL $INSTANCE_TYPE'
                }
            }
        }
    }
